...
public class maxconnect4
{
  public static void main(String[] args)
  {
    ...
    {
        System.out.println("File not found creating blank game.");
        int [][]masterGame = new int[6][7];
        for( int i = 0; i < 6; i++ )
    	{
    	    for( int j = 0; j < 7; j++)
    	    {
        ...
        currentGame = new GameBoard(masterGame);

    }

    // create the Ai Player
    AiPlayer calculon = new AiPlayer();
    AiPlayer.depthLimit = depthLevel;
    ...
    if( game_mode.equalsIgnoreCase( "interactive" ) )
    {
        ...
        if(!(turn.equalsIgnoreCase("computer-next")) && !(turn.equalsIgnoreCase("human-next")))
        {
            System.out.println("Incorrect arguements: need \"computer-next\" or \"human-next\"");
            System.exit(0);
        }

        // loop in interactive mode untill game is over
        while(currentGame.getPieceCount() < 42)
        {
            ...
            if(turn.equals("computer-next"))
            {
                int current_player = currentGame.getCurrentTurn();
            	// AI play - random play
                System.out.println("AI move:");

                playColumn = calculon.findBestMove( currentGame );
            	// play the piece
            ...
            else
            {
                int current_player = currentGame.getCurrentTurn();
                int move = -1;
                System.out.println("You are Player "+current_player);
            	// human play: ask for valid input and check if valid

                while(true)
                {
                    System.out.println("Make a valid move. (1 - 7)");
                    try
                    {
                    ...
                    if(move >= 1 && move <= 7 && currentGame.isValidPlay(move-1))
                    {
                        break;
                    }
                }
                ...
                // uncomment to test with random moves
                //System.out.println("RANDOM");
                //move = calculon.findRandom(currentGame);
